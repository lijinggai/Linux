    ...
    int ProcessCgi() //(重点)
    {
        ...
        else if (pid > 0)
        {
            ...
            if (method == "POST")
            {
                ...
                int total = 0;
                ssize_t s = 0;
                //std::cout<<getenv("ARGUMENTS_SIZE")<<" "<<post_arguments<<std::endl;
                ...
                std::cout<<post_arguments.c_str()<<std::endl;
            }
            // 父进程读取子进程的执行结果
            char ch;
...
public:
    ...
    void RecvRequest()
    {
        ...
        {
            //分析请求报头是否有content-length字段，来判断携带正文
            ParseRequestLine();
            ParseRequestHeader();
    ...
    void BuildResponse()
    {
        ...
        //如果请求方法是GET方法且不携带参数，不做CGI处理，直接返回静态网页
        if (method == "GET")
        {
...
public:
    void ProcessOn(int new_socket)
    {
        ...
        ep.RecvRequest();//读取和分析
        if (ep.GetStop() == false)
        {
            ep.BuildResponse();//处理请求 构建响应
            ep.SendResponse();//发送
        }
        else//读取失败
        {
            ep.BuildStatusLine(SERVER_ERR);
            ep.HandlerError(SERVER_ERR);
            ep.SendResponse();
        }
